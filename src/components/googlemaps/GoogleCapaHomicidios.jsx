...
const GoogleCapaHomicidios = ({ visible, filtros = null, map, google }) => {
  // Usar TanStack Query para obtener los datos con caché de 12 horas
  const {
    data: homicidiosFiltrados = [],
    isLoading: loading,
    error,
    isFetching
  } = useHomicidiosQuery(filtros, visible);
  ...
  // Memoizar el contenido del InfoWindow
    ...
    return `
          <div style="font-size: 13px; max-width: 260px; position: relative;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; border-bottom: 1px solid #eee; padding-bottom: 5px;">
              <strong style="color: #333;">Homicidio</strong>
  ...
  // Efecto principal para manejar los markers
  useEffect(() => {
    ...
    homicidiosFiltrados.forEach((item) => {
  ...
  }, [map, google, visible, homicidiosFiltrados, currentZoom, calcularRadio, createInfoWindowContent, limpiarMarkers]);
