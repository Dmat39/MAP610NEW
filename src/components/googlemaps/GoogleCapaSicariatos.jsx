...
const GoogleCapaSicariatos = ({ visible, filtros = null, map, google }) => {
  const {
    ...
    return `
          <div style="font-size: 13px; max-width: 260px; position: relative;">
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; border-bottom: 1px solid #eee; padding-bottom: 5px;">
              <strong style="color: #333;">Sicariato</strong>
  ...
  useEffect(() => {
    ...
    sicariatosFiltrados.forEach((item) => {
  ...
  }, [map, google, visible, sicariatosFiltrados, currentZoom, calcularRadio, createInfoWindowContent, limpiarMarkers]);

  return null;
};

export default memo(GoogleCapaSicariatos);
