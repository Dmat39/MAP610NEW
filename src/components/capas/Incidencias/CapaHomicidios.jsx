...
const CapaHomicidios = ({ visible, filtros = null }) => {
  ...
  // Los eventos de conteo se manejan automáticamente en el hook useHomicidiosQuery
    ...
    <>
      {(loading || isFetching) && (
        ...
        >
          ...
          >
            {loading ? 'Cargando homicidios...' : 'Actualizando...'}
          </span>
        </div>
      )}
      ...
      {error && (
        ...
        >
          Error cargando homicidios: {error.message}
        </div>
      )}

      <LayerGroup>
        {homicidiosFiltrados.map((item, idx) => {
...
export default React.memo(CapaHomicidios);
