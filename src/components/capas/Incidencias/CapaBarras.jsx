...
const CapaBarras = ({ visible, filtros = null }) => {
  // Usar TanStack Query para obtener los datos con caché de 12 horas
  const {
    data: barrasFiltrados = [],
    isLoading: loading,
    error,
    isFetching,
  } = useBarrasQuery(filtros, visible);

  // Los eventos de conteo se manejan automáticamente en el hook useBarrasQuery
    ...
    <>
      {(loading || isFetching) && (
        ...
        >
          ...
          >
            {loading ? 'Cargando incidencias de barras...' : 'Actualizando...'}
          </span>
        </div>
      )}
      ...
      {error && (
        ...
        >
          Error cargando barras: {error.message}
        </div>
      )}

      <LayerGroup>
        {barrasFiltrados.map((item, idx) => {
...
export default React.memo(CapaBarras);
