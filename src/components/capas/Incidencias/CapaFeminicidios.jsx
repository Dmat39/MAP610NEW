...
const CapaFeminicidios = ({ visible, filtros = null }) => {
  ...
  // Los eventos de conteo se manejan automáticamente en el hook useFeminicidiosQuery
    ...
    <>
      {(loading || isFetching) && (
        ...
        >
          ...
          >
            {loading ? 'Cargando feminicidios...' : 'Actualizando...'}
          </span>
        </div>
      )}
      ...
      {error && (
        ...
        >
          Error cargando feminicidios: {error.message}
        </div>
      )}

      <LayerGroup>
        {feminicidiosFiltrados.map((item, idx) => {
...
export default React.memo(CapaFeminicidios);
