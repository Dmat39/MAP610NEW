...
const CapaSecuestros = ({ visible, filtros = null }) => {
  // Usar TanStack Query para obtener los datos con caché de 12 horas
  const {
    data: secuestrosFiltrados = [],
    isLoading: loading,
    error,
    isFetching,
  } = useSecuestrosQuery(filtros, visible);

  // Los eventos de conteo se manejan automáticamente en el hook useSecuestrosQuery
    ...
    <>
      {(loading || isFetching) && (
        ...
        >
          ...
          >
            {loading ? 'Cargando secuestros...' : 'Actualizando...'}
          </span>
        </div>
      )}
      ...
      {error && (
        ...
        >
          Error cargando secuestros: {error.message}
        </div>
      )}

      <LayerGroup>
        {secuestrosFiltrados.map((item, idx) => {
...
export default React.memo(CapaSecuestros);
