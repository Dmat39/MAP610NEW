...
import { useSicariatosQuery } from '../../../hooks/useIncidenciasQuery';
...
const CapaSicariatos = ({ visible, filtros = null }) => {
  // Usar TanStack Query para obtener los datos con caché de 12 horas
  const {
    data: sicariatosFiltrados = [],
    isLoading: loading,
    error,
    isFetching,
  } = useSicariatosQuery(filtros, visible);

  // Los eventos de conteo se manejan automáticamente en el hook useSicariatosQuery
    ...
    <>
      {(loading || isFetching) && (
        ...
        >
          ...
          >
            {loading ? 'Cargando sicariatos...' : 'Actualizando...'}
          </span>
        </div>
      )}
      ...
      {error && (
        ...
        >
          Error cargando sicariatos: {error.message}
        </div>
      )}

      <LayerGroup>
        {sicariatosFiltrados.map((item, idx) => {
...
export default React.memo(CapaSicariatos);
